  ...
  agent{
    ...
    environment {
       ...
       //  When using returnStdout a trailing whitespace will be appended to the returned string. Use .trim() to remove this.
        SLACK_TOKEN='xoxb-5436971626113-5438090470150-uVmm7EuQoz0W5ubHvAW5k22g' //sh (script: "aws secretsmanager get-secret-value --secret-id jenkins --region us-east-1 | python3 -c \"import sys;import json;print(json.loads(json.loads(sys.stdin.read())['SecretString'])['slackToken'])\"", returnStdout: true).trim()
